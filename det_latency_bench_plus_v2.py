...
# -*- coding: utf-8 -*-
"""
det_latency_bench_plus_fix.py
-----------------------------
Fixes vs previous version:
 - EfficientDet now constructed with image_size=imgsz so anchors/FPN align (avoids tensor stack size error).
 - Uses torch.amp.autocast('cuda', ...) to avoid deprecation warnings.
 - Same CSV schema/output as before.

Families:
 - Ultralytics ("ultra"): YOLO/RT-DETR (.pt via ultralytics.YOLO) [PyTorch + TensorRT]
 - EfficientDet ("effdet"): tf_efficientdet_d{0..7} [PyTorch only]

Usage example:
python det_latency_bench_plus_fix.py \
...
def ensure_trtexec() -> str:
    ...
    if not exe:
        raise RuntimeError("`trtexec` not found in PATH. Install TensorRT and make sure trtexec is on PATH.")
    return exe
...
def bench_effdet_torch(model_name_or_ckpt: str, imgsz: int, mode: str, runs: int, warmup: int) -> BenchResult:
    """
    Build EfficientDet with anchors/FPN computed for the requested imgsz.
    This avoids size-mismatch errors like:
      'stack expects each tensor to be equal size, but got [1, 64, 5, 5] ... and [1, 64, 8, 8] ...'
    """
    ...
    # Construct model with the target input size
    if Path(model_name_or_ckpt).exists():
        # Load variant from filename, or default to d0
        ...
        model = EFF_CREATE(variant, bench_task="predict", pretrained=False, image_size=imgsz)
        ckpt = torch.load(model_name_or_ckpt, map_location="cpu")
        state = ckpt.get("state_dict", ckpt)
        model.load_state_dict(state, strict=False)
    else:
        # Named pretrained variant (weights are for default size, but effdet will rebuild heads for new image_size)
        model = EFF_CREATE(model_name_or_ckpt, bench_task="predict", pretrained=True, image_size=imgsz)
...
def main():
    ...
    ap.add_argument("--imgsz", nargs="+", type=int, default=[640], help="Square sizes to test, e.g. 320 512 640")
    ap.add_argument("--modes", nargs="+", choices=["torch", "fp16", "trt-fp16", "trt-int8"], default=["torch"])
    ap.add_argument("--runs", type=int, default=200)
    ap.add_argument("--warmup", type=int, default=50)
    ap.add_argument("--out", type=str, default="bench_results_plus_fix.csv")
    ap.add_argument("--workdir", type=str, default="bench_plus_artifacts_fix")
    args = ap.parse_args()
    ...
    for spec in args.models:
        ...
        for imgsz in args.imgsz:
            ...
            for mode in [m for m in args.modes if m in ("torch", "fp16")]:
                try:
                    if family == "ultra":
                        ...
                        # NOTE: EfficientDet requires consistent anchors/FPN for the chosen imgsz.
                        #       We build the model with image_size=imgsz to avoid size-mismatch.
                        if imgsz % 128 != 0:
                            log(f"[INFO] EfficientDet works best with image sizes multiple of 128; got {imgsz}.")
            ...
            # TensorRT (Ultralytics family only)
            if family == "ultra":
