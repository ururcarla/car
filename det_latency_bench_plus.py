...
# -*- coding: utf-8 -*-
"""
det_latency_bench_plus.py
-------------------------
Benchmark end-to-end per-frame latency for representative object detectors:
 - Ultralytics family ("ultra"): YOLOv5/8/10/RT-DETR .pt weights via ultralytics.YOLO
 - EfficientDet family ("effdet"): tf_efficientdet_d{0..7} via `effdet`

Supports:
 - PyTorch FP32 / AMP FP16 timing (end-to-end per-forward call)
 - TensorRT FP16/INT8 via trtexec (Ultralytics models only)
 - Resolution sweep, warmup, multiple runs
 - CSV results (same schema as yolo_latency_bench.py)

Examples
--------
python det_latency_bench_plus.py \
  --models yolov8n.pt rtdetr-l.pt tf_efficientdet_d0::effdet \
  --imgsz 320 640 --modes torch fp16 trt-fp16 \
  --runs 300 --warmup 100 --out results.csv
"""
...
def bench_ultra_torch(model_path: str, imgsz: int, mode: str, runs: int, warmup: int) -> BenchResult:
    ...
    return BenchResult(
        ...
        warmup=warmup,
        capture_ms=None,
        preprocess_ms=(pre_ms if 'pre_ms' in locals() else None),
...
def bench_effdet_torch(model_name_or_ckpt: str, imgsz: int, mode: str, runs: int, warmup: int) -> BenchResult:
    ...
    if Path(model_name_or_ckpt).exists():
        ...
        model = EFF_CREATE(variant, bench_task="predict", pretrained=False)
        ckpt = torch.load(model_name_or_ckpt, map_location="cpu")
        state = ckpt.get("state_dict", ckpt)
        model.load_state_dict(state, strict=False)
    else:
        model = EFF_CREATE(model_name_or_ckpt, bench_task="predict", pretrained=True)
...
def main():
    ...
    ap.add_argument("--imgsz", nargs="+", type=int, default=[640], help="Square sizes to test, e.g. 320 640")
    ap.add_argument("--modes", nargs="+", choices=["torch", "fp16", "trt-fp16", "trt-int8"], default=["torch"])
    ap.add_argument("--runs", type=int, default=200)
    ap.add_argument("--warmup", type=int, default=50)
    ap.add_argument("--out", type=str, default="bench_results_plus.csv")
    ap.add_argument("--workdir", type=str, default="bench_plus_artifacts")
    args = ap.parse_args()
