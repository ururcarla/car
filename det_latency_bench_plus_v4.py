...
# -*- coding: utf-8 -*-
"""
det_latency_bench_plus_v4.py
----------------------------
What's new:
 - EfficientDet path made robust:
    * First try DetBenchPredict(image_size=(H,W)).
    * If it raises (e.g., unpack/anchor/FPN issues), automatically fall back to the base EfficientDet forward
      and time compute-only (no postprocess/NMS). Notes column will reflect which path was used.
 - Keeps AMP fix: torch.amp.autocast("cuda", dtype=torch.float16).
 - CSV schema unchanged.

Families:
 - "ultra": Ultralytics (YOLO/RT-DETR) via ultralytics.YOLO  [PyTorch + TensorRT]
 - "effdet": EfficientDet via effdet                      [PyTorch (bench or base forward)]

Example:
python det_latency_bench_plus_v4.py \
  --models yolov8n.pt rtdetr-l.pt tf_efficientdet_d0::effdet \
  --imgsz 512 --modes torch fp16 --runs 200 --warmup 50 --out results.csv
"""
...
def _effdet_input_tensor(imgsz: int, device: str):
    img = np.random.randint(0, 255, (imgsz, imgsz, 3), dtype=np.uint8)
    if cv2 is not None:
        img = cv2.resize(img, (imgsz, imgsz), interpolation=cv2.INTER_LINEAR)
...
def bench_effdet_torch(model_name_or_ckpt: str, imgsz: int, mode: str, runs: int, warmup: int) -> BenchResult:
    if EFF_CREATE is None or torch is None:
        raise RuntimeError("effdet and/or torch not installed. `pip install effdet torch`")
    ensure_cuda()
    device = "cuda"
    image_size = (imgsz, imgsz)

    # Try DetBenchPredict first
    bench = None
    path = Path(model_name_or_ckpt)
    try:
        if path.exists():
            variant = "tf_efficientdet_d0"
            m = re.search(r"(tf_)?efficientdet_[dD](\d)", path.stem)
            if m:
        ...
        bench = EFF_BENCH(model).eval().to(device)
        mode_notes = "EfficientDet DetBenchPredict (includes postprocess)"
    except Exception as e:
        log(f"[INFO] DetBenchPredict path failed: {e}. Falling back to base forward (no NMS).")
        bench = None
        # Build base model (no bench), compute-only
        if path.exists():
            variant = "tf_efficientdet_d0"
            m = re.search(r"(tf_)?efficientdet_[dD](\d)", path.stem)
            if m:
                variant = f"tf_efficientdet_d{m.group(2)}"
            model = EFF_CREATE(variant, bench_task=None, pretrained=False, image_size=image_size)
            ckpt = torch.load(model_name_or_ckpt, map_location="cpu")
            state = ckpt.get("state_dict", ckpt)
            model.load_state_dict(state, strict=False)
        else:
            model = EFF_CREATE(model_name_or_ckpt, bench_task=None, pretrained=True, image_size=image_size)
        model.eval().to(device)
        mode_notes = "EfficientDet base forward (no postprocess)"

    tensor = _effdet_input_tensor(imgsz, device)
    use_amp = (mode == "fp16")
    ...
    with torch.inference_mode():
        for _ in range(warmup):
            if use_amp:
                with torch.amp.autocast("cuda", dtype=torch.float16):
                    _ = bench(tensor) if bench is not None else model(tensor)
            else:
                _ = bench(tensor) if bench is not None else model(tensor)
    ...
    return BenchResult(
        ...
        preprocess_ms=None,
        infer_ms_mean=mean,
        nms_ms=None if bench is None else 0.0,  # unknown; set 0 for bench path to keep schema
        end2end_ms_mean=mean,
        end2end_ms_p50=percentile(e2e, 0.5),
        end2end_ms_p90=percentile(e2e, 0.9),
        end2end_ms_p95=percentile(e2e, 0.95),
        end2end_ms_p99=percentile(e2e, 0.99),
...
def main():
    ...
    ap.add_argument("--out", type=str, default="bench_results_plus_v4.csv")
    ap.add_argument("--workdir", type=str, default="bench_plus_artifacts_v4")
    args = ap.parse_args()
